...
//Atributos
	...
	private JLabel lblNewLabel_6 = new JLabel("Rango de dirección del host:");
	private JLabel label4 = new JLabel("Clase de la dirección:");
	...
	private void initialize() {
		...
		frmCalculad.getContentPane().add(BtnIPv4);
		...
		frmCalculad.getContentPane().add(BtnIPv6);
	...
	private void btnGuardarActionPerformed ( ActionEvent e) {
		try {
			...
			database.dbPrepareStatementInt(integer, "INSERT INTO ip (octeto1,octeto2,octeto3,octeto4,cidr,subredes,hostporsubred)"+"VALUES (?,?,?,?,?,?,?)");
			database.dbPrepareStatement(string,"INSERT INTO ip (direcciondered,direccionbroadcast,clasedireccion,MascaraSubred,rangohost) " + "VALUES (?,?,?,?)" );
	...
	private void BtnIPv6Mostrar (ActionEvent e) {
		frmCalculad.dispose();
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					IPv6Frame window = new IPv6Frame();
					window.frmCalculadoraDeDirecciones.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
